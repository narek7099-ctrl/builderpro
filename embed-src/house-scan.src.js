/* HOUSE SCAN — the map-to-house shot for every calculator except roofing.
   ...
   own way, all in the same white-block look:
     ...
     pools        asked first: shape, size, where. The pool is dug and filled
                  in that spot
     landscaping  asked first: which yard, how big. That yard is laid out
   ...
   Nothing here is coloured by the homeowner's choices except pools and
   landscaping — how things will LOOK is the image generator's job.
...
/*@@SHARED@@*/
  ...
  var FINALE = {
    hvac: function(T,H,now){
      ...
      var ca = sa!==0 ? (sa>0?H.a1+4.2:H.a0-4.2) : (H.a0+H.a1)/2 - H.front[0]*(H.a1-H.a0)*0.15;
      var cb = sb!==0 ? (sb>0?H.b1+4.2:H.b0-4.2) : (H.b0+H.b1)/2 - H.front[1]*(H.b1-H.b0)*0.15;
      var rise=ease(span(T,8.4,9.4)), top=0.35+3.2*rise;
      var faces=[].concat(
        box(H, ca-2.2,ca+2.2, 0,0.35, cb-2.2,cb+2.2, 'pad', {fill:[236,240,245]}),
        rise>0.02 ? box(H, ca-1.8,ca+1.8, 0.35,top, cb-1.8,cb+1.8, 'unit', {fill:[250,251,253], top:'fan'}) : []);
      var camP=ease(span(T,10.0,12.2));
      return {
        faces:faces, houseA:1-0.7*camP,
        face2: faceTo(H,ca,cb),
        cam: camP>0 ? {c:H.w(ca,2,cb), R:14, H:6, p:camP} : null,
        face: function(ctx,P,F,dpr){
          ...
          var ang=now*7, R=1.45, y=top+0.01, pts=[];
        ...
        over: function(ctx,P,dpr){
          ...
          chain(ctx,P,[[H.w(ca-sa*1.8,1.6,cb-sb*1.8),H.w(wa,1.6,wb)],[H.w(wa,1.6,wb),H.w(wa,7,wb)]], span(T,9.6,10.6), BLUE, 2*dpr);
          ctx.restore();
        }
      };
    },
    ...
    pools: function(T,H){
      var q=st.q||{}, poly=poolPoly(q), loc=q.where||'back';
      ...
      return { faces:[], houseA:1, face2: faceTo(H,ca,cb),
        ...
        under: function(ctx,P,dpr){
          ...
          if(depth>0.05){
            ...
            ctx.fillStyle='#e6ebf1'; ctx.beginPath(); bot.map(P).forEach(function(s,i){ i?ctx.lineTo(s.x,s.y):ctx.moveTo(s.x,s.y); }); ctx.closePath(); ctx.fill();
            for(var j=0;j<top.length;j++){ var k2=(j+1)%top.length;
              ...
              ctx.fillStyle='#d9e0e8'; ctx.beginPath(); quad.forEach(function(s,i){ i?ctx.lineTo(s.x,s.y):ctx.moveTo(s.x,s.y); }); ctx.closePath(); ctx.fill(); }
            if(fill>0){ var wy=-depth+(depth-0.6)*fill, water=ringPts(H,ring,wy).map(P);
              ctx.fillStyle='rgba(120,190,255,'+(0.55*fill+0.15)+')'; ctx.beginPath(); water.forEach(function(s,i){ i?ctx.lineTo(s.x,s.y):ctx.moveTo(s.x,s.y); }); ctx.closePath(); ctx.fill(); }
          }
          ctx.restore();
    ...
    landscaping: function(T,H){
      var q=st.q||{}, where=q.where||'back', size=q.size||1200;
      var patches=[];
      ...
      return { faces:[], houseA:1, face2: faceTo(H,cen[0],cen[1]),
        ...
        under: function(ctx,P,dpr,now){
          patches.forEach(function(r,pi){
            ...
            if(g>0){ ctx.save(); ctx.globalAlpha=g; ctx.fillStyle='#e3efdc'; ctx.beginPath(); scr.forEach(function(s,i){ i?ctx.lineTo(s.x,s.y):ctx.moveTo(s.x,s.y); }); ctx.closePath(); ctx.fill(); ctx.restore(); }
            ...
            for(var s=0;s<7;s++){ var t=(s+0.5)/7, pop=ease(span(T,10.8+s*0.15+pi*0.3,11.3+s*0.15+pi*0.3)); if(pop<=0) continue;
              ...
              ctx.fillStyle='#9fc98f'; ctx.beginPath(); ctx.arc(sc.x,sc.y,rr,0,Math.PI*2); ctx.fill();
              ctx.fillStyle='#b8d9aa'; ctx.beginPath(); ctx.arc(sc.x-rr*0.3,sc.y-rr*0.3,rr*0.55,0,Math.PI*2); ctx.fill(); }
            for(var k=0;k<5;k++){ var t2=(k+0.5)/5, on=span(T,11.8+k*0.12+pi*0.3,12.2+k*0.12+pi*0.3); if(on<=0) continue;
  ...
  function draw(){
    ...
    var geo=null;
    if(st.target){ geo=st.planes? st.planes.geo : st.gen; if(st.arrive===undefined) st.arrive=T; }
  ...
  /* ── pool and landscaping ask before the shot, so the shot can show it ─────── */
  var QUESTIONS = {
    pools: [
      {k:'shape', t:'Pool shape', o:[['rect','Rectangle'],['freeform','Freeform'],['kidney','Kidney'],['l','L-shape']]},
      {k:'size',  t:'Size',       o:[[288,'Small · 12×24'],[512,'Medium · 16×32'],[800,'Large · 20×40']]},
      {k:'where', t:'Where',      o:[['back','Back yard'],['side','Side yard'],['front','Front yard']]}
    ],
    landscaping: [
      {k:'where', t:'Which yard', o:[['back','Back yard'],['front','Front yard'],['both','Both']]},
      {k:'size',  t:'About how big', o:[[600,'Small · ~600 sq ft'],[1500,'Medium · ~1,500'],[3000,'Large · ~3,000']]}
    ]
  };
  function askFirst(go){
    var qs=QUESTIONS[TRADE]; if(!qs){ go(); return; }
    var box=$('hs-q'), ans={};
    qs.forEach(function(q){ ans[q.k]=q.o[q.k==='size'?1:0][0]; });
    box.innerHTML=qs.map(function(q){
      return '<div class="hs-qrow"><div class="hs-qt">'+q.t+'</div><div class="hs-qo">'
        +q.o.map(function(o,i){ return '<button type="button" class="hs-chip'+(ans[q.k]===o[0]?' on':'')+'" data-k="'+q.k+'" data-v="'+o[0]+'">'+o[1]+'</button>'; }).join('')
        +'</div></div>';
    }).join('')+'<button type="button" class="hs-go" id="hs-go">'+(TRADE==='pools'?'Show my pool':'Show my yard')+'</button>';
    box.className='hs-q on';
    $('hs-canvas').style.display='none'; $('hs-livepanel').style.display='none';
    box.querySelectorAll('.hs-chip').forEach(function(b){ b.onclick=function(){
      var k=b.getAttribute('data-k'), v=b.getAttribute('data-v'); ans[k]=isNaN(+v)?v:+v;
      box.querySelectorAll('.hs-chip[data-k="'+k+'"]').forEach(function(x){ x.classList.toggle('on', x===b); });
    }; });
    $('hs-go').onclick=function(){ box.className='hs-q'; box.innerHTML=''; $('hs-canvas').style.display=''; $('hs-livepanel').style.display=''; go(ans); };
  }
  ...
  function begin(addr){
    ...
    st={ cv:cv, ctx:cv.getContext('2d'), mcv:mcv, mctx:mcv.getContext('2d'), dpr:Math.min(2, window.devicePixelRatio||1),
         t0:performance.now(), clock:0, idx:-1, data:undefined, target:null, waiting:true };
  ...
  var RESULT = {
    ...
    pools:       function(d,q){ return [[fmt(q.size),'Pool surface, sq ft'],[({back:'Back',side:'Side',front:'Front'})[q.where]+' yard','Location'],[fmt(d.planSqft),'Home footprint, sq ft']]; },
    landscaping: function(d,q){ return [[fmt(q.size),'Yard to landscape, sq ft'],[({back:'Back yard',front:'Front yard',both:'Front & back'})[q.where],'Where'],[fmt(d.planSqft),'Home footprint, sq ft']]; },
  ...
  var PREFILL = {
    ...
    pools:function(d,q){ return q.size; }, landscaping:function(d,q){ return q.size; }
  };
  ...
  function handoff(data, q){
    ...
    var v = (data&&PREFILL[TRADE]) ? PREFILL[TRADE](data,q) : (PREFILL[TRADE]&&(TRADE==='pools'||TRADE==='landscaping') ? PREFILL[TRADE]({},q) : null);
