...
/*@@SHARED@@*/
  ...
  function hvacUnit(T,H,now){
    ...
    function louvres(s,from,to){ return function(ctx,P,dpr){ if(det<=0) return;
      ...
      for(var y=0.95;y<=top-0.3;y+=0.2) ln(ctx,P,side(s,from,y),side(s,to,y));
    ...
    function fan(ctx,P,dpr){ if(k3<0.5) return;
      var y=lid+0.005, ang=now*6, ring=function(r){ var pts=[]; for(var i=0;i<40;i++){ var t=i/40*Math.PI*2; pts.push(H.w(Math.cos(t)*r,y,Math.sin(t)*r)); } return pts; };
      ...
      ctx.strokeStyle=LINE; [1.75,1.4,1.05,0.7].forEach(function(r){ loop(ctx,P,ring(r),null); });
      for(var sp=0;sp<12;sp++){ var t2=sp/12*Math.PI*2; ln(ctx,P,H.w(Math.cos(t2)*0.36,y,Math.sin(t2)*0.36),H.w(Math.cos(t2)*1.75,y,Math.sin(t2)*1.75)); }
  ...
  function kitchen(T,H,now){
    ...
    function doors(axis,fixed,from,to,y0,y1,drawer){ return function(ctx,P,dpr){ if(det<=0) return; ctx.save(); ctx.globalAlpha*=det; ctx.strokeStyle=LINE; ctx.lineWidth=1*dpr;
      ...
      for(var t=from+1.5;t<to-0.3;t+=1.5) ln(ctx,P,pt(t,y0+0.25),pt(t,y1-0.1));
      if(drawer) ln(ctx,P,pt(from+0.08,y1-0.55),pt(to-0.08,y1-0.55));
      ctx.strokeStyle=LINE2; ctx.lineWidth=2*dpr; for(var t2=from+0.75;t2<to;t2+=1.5) ln(ctx,P,pt(t2-0.25,y1-(drawer?0.75:0.4)),pt(t2+0.25,y1-(drawer?0.75:0.4)));
      ctx.restore(); }; }
    ...
    function cooktop(ctx,P,dpr){ if(det<=0) return; ctx.save(); ctx.globalAlpha*=det; ctx.strokeStyle=LINE2; ctx.lineWidth=1.2*dpr; var y=sh+0.005;
      [[-5.4,-1.6],[-4.6,-1.6],[-5.4,-0.6],[-4.6,-0.6]].forEach(function(c){ var pts=[]; for(var i=0;i<24;i++){ var t=i/24*Math.PI*2; pts.push(H.w(c[0]+Math.cos(t)*0.3,y,c[1]+Math.sin(t)*0.3)); } loop(ctx,P,pts,null); });
      ctx.restore(); }
  ...
  function room(T,H,now,mode){
    ...
    var cen=H.w(0,4,0);
    function cull(dirA,dirB){ return function(P){ var c=P(H.w(dirA*A,4,dirB*B)), o=P(H.w(dirA*(A+3),4,dirB*(B+3))); return o.z<c.z; }; }
  ...
  function draw(){
    ...
    if(houseA>=0.999){
      faces.forEach(function(F){ if(F.extra&&F.f.cull&&F.f.cull(F.PP)) return; var a=F.a*(F.extra?1:growA); if(a>0.01) paint(ctx,F,a); });
    } else {
      ...
      faces.forEach(function(F){ if(F.extra&&!(F.f.cull&&F.f.cull(F.PP))) paint(ctx,F,1); });
    }
